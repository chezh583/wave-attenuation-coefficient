...
len=length(dic);
out_ki=zeros(25,len);
out_ki_simple=zeros(25,len);
cut_num=zeros(1,len);
load('interperation_results/buoylist1.mat'); %dim: buoy_frq,num
load('interperation_results/buoylist2.mat');
load('output/Ef_step6.mat');
%========================
method=2;
%========================
%%
i=187;
load('output/out_ki.mat');
load('output/out_ki_simple.mat');
temparray=importdata(['interpolated_variables/',num2str(dic(i)),'/sic.txt']);
cut=length(temparray);
Ef=zeros(25,cut);
Si=Ef; Sds=Ef; Snl=Ef;
A=log10(Ef_step6(:,1,i));
B=log10(Ef_step6(:,2,i));
for j=1:cut
...
ylabel('$S_{in}$ (m$^2$/Hz/s)','fontsize',16,'FontName','times','Interpreter','latex')
ylim([-2e-7 2e-7])
...
xlim([0.05 0.25])
ylabel('$S_{nl}$ (m$^2$/Hz/s)','fontsize',16,'FontName','times','Interpreter','latex')
...
% 
 ...
 for i=1:len
     if(disparity(i))
         ...
         for j=1:cut
             fname=[source_dir,num2str(dic(i)),'_cut',num2str(j),'.nc'];
             % efth=ncread(['interpolated_variables/',num2str(dic(i)),'/cut',num2str(j),'.nc'],'efth');
             % efth=squeeze(efth(:,:,end,end));
             % efth=sum(efth);
             % Ef(:,j)=efth';
             alpha= (j-1) / (cut-1);
             interp_wvsp=(1 - alpha) * A + alpha * B;
...
buoyspec1=importdata(['interperation_results/buoylist1.mat']);  % wave spectrum collected by buoy1
buoyspec2=importdata(['interperation_results/buoylist2.mat']);  % wave spectrum collected by buoy2
position=importdata(['output/position.mat']);

alpha=zeros(25,len);
onealpha=zeros(len,1);
D=position(:,3);
dic=num2str(dic);
for i=1:len
    if(disparity(i))
    ...
    onealpha(i)=log(trapz(frq,b1)./trapz(frq,b2))/(2*D(inum));
    end
end
